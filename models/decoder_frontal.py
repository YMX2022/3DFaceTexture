...
class ReconstructionLayer(nn.Module):
    ...
    def forward(self, params, detail_tex):
        b = detail_tex.shape[0]
        ...
        shape = self.proxy_model.shape_mean.reshape(b,-1,3)
        coarse_tex = self.texFormation(params_tex)
#         shape_rt = self.rigidTrans(shape, params_rot, params_xyz)
        # Render
        frontal_image = self.face_renderer(shape, detail_tex)
