...
class ReconstructionLayer(nn.Module):
    ...
    def forward(self, params, img_tex, img_mask):
        b = params.shape[0]
        params_id, params_ex, params_tex, params_rot, params_illu, params_xyz = self.splitParams(params)
        # 3dmm decode
        shape_standard = self.proxy_model.shape_mean.reshape(1, -1, 3).repeate(b,1,1)
        # shape gen
        shape_rt = self.rigidTrans(shape_standard, params_rot, params_xyz)
        return self.renderer(shape_rt, img_tex * img_mask)