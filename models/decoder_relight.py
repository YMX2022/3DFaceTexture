...
class BFM09(FaceModelBase):
    ...
    def parse_file(self):
        ...
        self.register_buffer('point_buf', torch.from_numpy(getattr(self.reader,
                                                                   'point_buf') - 1).long())  # adjacent face index for each vertex, starts from 1(used for calculate vertex normal)
        self.register_buffer('kps', torch.from_numpy(np.squeeze(getattr(self.reader, 'keypoints')).astype(
            np.int32) - 1).long())  # vertex index for all the landmark points
        self.register_buffer('triangles', torch.from_numpy(
            getattr(self.reader, 'tri') - 1).long())  # vertex index for all the triangles, start from 1
...
class ReconstructionLayer(nn.Module):
    ...
    def shapeFormation(self, id_params, ex_params):
        ...
        ## re-center faceshape
        face_shape = face_shape.reshape(face_shape.size()[0], -1, 3) - torch.mean(shape_mean.reshape(1, -1, 3), dim=1,
                                                                                  keepdim=True)
        return face_shape
    ...
    def rotationFormation(self, trans_params):
        ...
        rot_x = torch.stack(
            [ones, zeros, zeros, zeros, torch.cos(angle_x), -torch.sin(angle_x), zeros, torch.sin(angle_x),
             torch.cos(angle_x)], dim=-1).reshape(-1, 3, 3)
        rot_y = torch.stack(
            [torch.cos(angle_y), zeros, torch.sin(angle_y), zeros, ones, zeros, -torch.sin(angle_y), zeros,
             torch.cos(angle_y)], dim=-1).reshape(-1, 3, 3)
        rot_z = torch.stack(
            [torch.cos(angle_z), -torch.sin(angle_z), zeros, torch.sin(angle_z), torch.cos(angle_z), zeros, zeros,
             zeros, ones], dim=-1).reshape(-1, 3, 3)
    ...
    def illuFormation(self, illu_params, face_norms, diffuse_albedo):
        ...
        device = illu_params.device
        ones = torch.ones(face_norms.shape[0] * face_norms.shape[1], dtype=torch.float32).to(device)
        transfer_list = []
    ...
    def computeNorm(self, face_shape, face_model):
        ...
        face_norm = torch.cat([face_norm, torch.zeros(face_norm.shape[0], 1, 3, dtype=torch.float32, device=device)],
                              dim=1)
    ...
    def register_constants(self):
        ...
        cam_mat = torch.from_numpy(
            np.array([self.focal, 0, half_w, 0, -self.focal, half_w, 0, 0, 1]).reshape(3, 3)).unsqueeze(0).float()
    ...
    def forward(self, params, detail_tex, params_illu=None):
        if params_illu is None:
            params_id, params_ex, _, params_rot, params_illu, params_xyz = self.splitParams(params)
        else:
            params_id, params_ex, _, params_rot, _, params_xyz = self.splitParams(params)
        # 3dmm decode
        shape = self.shapeFormation(params_id, params_ex)
        norm = self.computeNorm(shape, self.proxy_model)
        tex_illu = self.illuFormation(params_illu, norm, detail_tex * 255.0) / 255.0
        shape_rt = self.rigidTrans(shape, params_rot, params_xyz)
        images = self.face_renderer(shape_rt, tex_illu)
        return images
