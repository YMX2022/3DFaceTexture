...
class FaceRenderer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config['proxy'] == 'BFM09':
            self.register_buffer('triangles', BFM09(self.config['proxy_path']).triangles.long().contiguous())
        ...
        self.register_buffer('light_positions',torch.as_tensor([0,0.3,1e5]).float()[None,None,:])
        self.register_buffer('light_intensities',torch.ones(1,3).float()[None,...])
...
class ReconstructionLayer(nn.Module):
    ...
    def forward(self, params, detail_tex, light=None):
        ...
        self.detail_tex_face_illu = self.illuFormation(params_illu, self.norm, detail_tex * 255.0) / 255.0
        self.coarse_illu = self.illuFormation(params_illu, self.norm, torch.ones_like(coarse_tex) * 180) / 255.0
        if light is not None:
            self.detail_illu = self.illuFormationUpdate(light, self.norm, torch.ones_like(coarse_tex) * 180) / 255.0
            self.detail_tex_face_illu_update = self.illuFormationUpdate(light, self.norm, detail_tex * 255.0) / 255.0            
        # shape gen
        self.shape_rt = self.rigidTrans(shape, params_rot, params_xyz)
        detail_face_image = self.face_renderer(self.shape_rt, self.detail_tex_face_illu)
        detail_face_image_with_illu = self.face_renderer(self.shape_rt, detail_tex)
        return detail_face_image, detail_face_image_with_illu